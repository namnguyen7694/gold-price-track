...
export async function performGoldCrawl() {
  const [localGoldList, worldPriceUsd, exchangeRate] = await Promise.all([
    scrapeLocalGoldPrice(),
    scrapeWorldGoldPrice(),
    getExchangeRate(),
  ]);

  if (!localGoldList || !worldPriceUsd || !exchangeRate) {
    throw new Error('Failed to scrape one or more sources');
  }

  // Find "Nhẫn 999.9" as the primary local price
  const primaryLocal = localGoldList.find((item: { name: string }) => item.name.includes('Nhẫn 999.9')) || localGoldList[0];
